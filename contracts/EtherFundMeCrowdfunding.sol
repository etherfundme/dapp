...
/// @title etherfund.me  crowdfunding service contract
contract EtherFundMeCrowdfunding {
  ...
  mapping (address => uint256) public investedAmountOf;
	...
	/// etherfund.me service fee in %
  uint public constant SERVICE_FEE = 4;
...
function isGoalReached() public constant returns (bool reached) {
 ...
 function invest() public payable stopInEmergency  {
	 ...
	 // Update investor
	 investedAmountOf[investor] += weiAmount;
 ...
 function finalize() public inState(State.Success) stopInEmergency  {
	 ...
	 uint feeAmount = (this.balance * SERVICE_FEE) / 100;
